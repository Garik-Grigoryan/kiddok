    ...
    <v-row justify="center">
      <v-col cols="6">
        ...
        <v-form ref="form" v-model="valid" >
          ...
          <v-btn :disabled="!valid" @click="addPartner" color="success" class="mr-4" >Save</v-btn>
        </v-form>
      </v-col>
    </v-row>
    ...
    methods: {
      ...
      addPartner() {
        if(this.name !== "" && this.link !== "" && this.selectedImages.length !== 0) {
          this.$store.dispatch('partners/addPartner', [this.name, this.link, this.selectedImages]).then(r => {
